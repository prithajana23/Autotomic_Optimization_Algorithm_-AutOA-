...
# --- IMPORTANT: KAGGLE DATA PATH ---
# Update this if running on Kaggle
...
def load_and_preprocess_data():
    ...
    if not os.path.exists(DATA_FILE):
        print(f"Data file '{DATA_FILE}' missing.")
        # Fallback for local testing if file is missing
        raise FileNotFoundError(f"Please check path: {DATA_FILE}")
...
def run_optimizer(optimizer_func, run_num):
    ...
    try:
        # This function MUST populate the convergence_history list
...
def run_selective_aoa(history_list):
    """
    Wrapper for Selective Autotomic AOA (S-AutOA) adapted for HPO.
    """
    # --- Configuration ---
    ...
    # L-SHADE / S-AutOA Config
    # For HPO, population is fixed (unlike high-dim benchmark), 
    # but we will use the same S-AutOA logic.
    
    pop = np.random.uniform(lb, ub, (pop_size, dim))
    fit = np.array([objective_function(ind) for ind in pop])
    ...
    # Add initial bests to history (one for each eval)
    # Note: L-SHADE creates many evals per gen. We just append best_fit.
    history_list.extend([best_fit] * pop_size)
    ...
    while fevals < max_fevals:
        ...
        if len(dead_dims) > 0:
            ...
            for k in range(start_idx, pop_size):
                ...
                # Apply OBL to Dead Dimensions
                jitter = np.random.normal(0, 1e-3, len(dead_dims))
                repair_vector[dead_dims] = (lb[dead_dims] + ub[dead_dims]) - repair_vector[dead_dims] + jitter
                
                repair_vector = np.clip(repair_vector, lb, ub)
                
                f_new = objective_function(repair_vector)
                fevals += 1
                history_list.append(min(f_new, best_fit))
                ...
                if f_new < best_fit:
                    best_fit = f_new
                    best_sol = repair_vector.copy()

        # 3. Evolution (L-SHADE Style)
        pop_new, SCR, SF = [], [], []
        ...
        for i in range(pop_size):
            ...
            F = np.clip(np.random.standard_cauchy() * 0.1 + M_F[r_idx], 0, 1) # Cauchy
        ...
        if not pop_new: break
        
        pop_new = np.array(pop_new)
        # Evaluate one by one for HPO to respect FE budget strictly
        fit_new_vals = []
        for ind in pop_new:
            if fevals < max_fevals:
                val = objective_function(ind)
                fevals += 1
                fit_new_vals.append(val)
                if val < best_fit:
                    best_fit = val
                    best_sol = ind.copy()
                history_list.append(best_fit)
            else:
                fit_new_vals.append(np.inf) # Dummy value
        
        fit_new_vals = np.array(fit_new_vals)
...
def main():
    ...
    optimizers = {
        ...
        "Selective AOA": run_selective_aoa,
    }
