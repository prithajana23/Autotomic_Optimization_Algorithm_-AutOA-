...
#
# CMA-ES
def run_cma(history_list):
    lb, ub, dim = HPO_BOUNDS_LOW, HPO_BOUNDS_HIGH, HPO_DIM
    
    # Initial guess (center of the [0,1] space) and step size
    es = cma.CMAEvolutionStrategy(
        dim * [0.5], 
        0.5, # Initial standard deviation
        {
            'bounds': [lb, ub], 
            'popsize': POP_SIZE,
    ...
    actual_popsize = es.popsize # Get the actual popsize CMA decided to use
    ...
    while not es.stop():
        ...
        # 2. Evaluate them
        fitnesses = [objective_function(s) for s in solutions]
        
        # 3. Tell CMA the results
        es.tell(solutions, fitnesses)
        
        # 4. Update history
        # es.result.fbest is the best-so-far score
        if es.result.fbest < current_best_loss:
            current_best_loss = es.result.fbest
            
        # Add the current best score once for each evaluation in this generation
        for _ in range(actual_popsize):
             history_list.append(current_best_loss)
             
    # Ensure history list is exactly MAX_FEVALS long
    history_list[:] = history_list[:MAX_FEVALS]
