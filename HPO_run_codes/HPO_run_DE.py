...
def run_de(history_list):
    
    # This list will be populated by the wrapper function below
    # It stores the best-so-far loss after each *evaluation*
    run_eval_history = []

    def objective_wrapper_de(cont_params):
        """A wrapper for the objective function to track history per-evaluation."""
        # 1. Call the expensive objective function
        loss = objective_function(cont_params)
        
        # 2. Update and append the best-so-far loss
        if not run_eval_history:
            run_eval_history.append(loss)
        else:
            # Append the minimum of the current loss and the previous best loss
            run_eval_history.append(min(loss, run_eval_history[-1]))
            
        return loss

    # 3. Call the optimizer
    # We set maxiter = (MAX_FEVALS / POP_SIZE) - 1
    # This is because DE does one initial population evaluation (POP_SIZE evals)
    # and then maxiter * POP_SIZE evaluations.
    # e.g., (50/10)-1 = 4 iterations. Total evals = (4+1) * 10 = 50.
    result = differential_evolution(
        objective_wrapper_de, 
        bounds=list(zip(HPO_BOUNDS_LOW, HPO_BOUNDS_HIGH)), 
        strategy='best1bin',
        maxiter=int(MAX_FEVALS / POP_SIZE) - 1,
        popsize=POP_SIZE,
        callback=None, # We don't need a callback, the wrapper handles history
        polish=False,
        seed=np.random.randint(1, 100000)
    )

    # 4. Copy the recorded history into the main list
    # The wrapper ensures `run_eval_history` has exactly MAX_FEVALS items
    history_list[:] = run_eval_history
...
def main():
    ...
    optimizers = {
        ...
        "DE (best1bin)": run_de,  # <-- ADD THIS LINE
