...
def run_gwo(history_list):
    ...
    # 2. Initial evaluation and history update
    for i in range(pop_size):
        ...
        if Fitness[i] < Alpha_score:
            ...
            Delta_score, Delta_pos = Beta_score, Beta_pos.copy()
            # Promote new Beta
            Beta_score, Beta_pos = Fitness[i], Population[i].copy()
        elif Fitness[i] < Delta_score:
            # Promote new Delta
            Delta_score, Delta_pos = Fitness[i], Population[i].copy()
            
        history_list.append(Alpha_score) # Append best-so-far after each eval

    # 3. Main optimization loop
    for t in range(max_iter - 1): # -1 because we did one iteration (initialization)
        a = 2 - t * 2 / max_iter  # a decreases linearly from 2 to 0

        # Update each wolf in the population
        for i in range(pop_size):
            ...
            for leader_pos in [Alpha_pos, Beta_pos, Delta_pos]:
                ...
                A = 2 * a * r1 - a  # A vector
                C = 2 * r2          # C vector
                D = abs(C * leader_pos - Population[i]) # Distance
                X.append(leader_pos - A * D)
            
            # Average the 3 leader-guided positions
