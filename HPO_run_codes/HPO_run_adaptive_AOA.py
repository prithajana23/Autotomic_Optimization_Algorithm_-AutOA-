...
#
# Adaptive AOA
def run_adaptive_aoa(history_list, alpha):
    ...
    for _ in range(max_iter - 1): # -1 for initial eval
        div = diversity(Population)
        
        # New generation (we copy to avoid in-place modification issues)
        new_population = Population.copy()
        new_fitness = Fitness.copy()

        # Strategy 1: Exploration (High Diversity)
        if div > div_threshold:
            for i in range(pop_size):
                # Exploration: Mutate based on best solution
...
def main():
    ...
    optimizers = {
        ...
        "Adaptive AOA t=0.1": lambda h: run_adaptive_aoa(h, alpha=0.1),
        "Adaptive AOA t=0.2": lambda h: run_adaptive_aoa(h, alpha=0.2),
        "Adaptive AOA t=0.3": lambda h: run_adaptive_aoa(h, alpha=0.3),
        "Adaptive AOA t=0.4": lambda h: run_adaptive_aoa(h, alpha=0.4),
        "Adaptive AOA t=0.5": lambda h: run_adaptive_aoa(h, alpha=0.5),
        "Adaptive AOA t=0.6": lambda h: run_adaptive_aoa(h, alpha=0.6),
        "Adaptive AOA t=0.7": lambda h: run_adaptive_aoa(h, alpha=0.7),
        "Adaptive AOA t=0.8": lambda h: run_adaptive_aoa(h, alpha=0.8),
        "Adaptive AOA t=0.9": lambda h: run_adaptive_aoa(h, alpha=0.9),
