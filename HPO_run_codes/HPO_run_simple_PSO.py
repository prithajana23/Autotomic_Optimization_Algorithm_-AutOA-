...
import numpy as np # Already imported, just showing dependency
...
class SimplePSO:
    def __init__(self, obj_func, dim, bounds, num_particles=10, max_iters=50,
                 c1=1.5, c2=1.5, w=0.7):
        ...
        self.convergence_history_per_iter = [] # Stores best score *per iteration*
    ...
    def optimize(self):
        ...
        self.convergence_history_per_iter.append(self.global_best_score)

        for iter_no in range(self.max_iters - 1): # -1 because we did initial eval
            for particle in self.particles:
        ...
        return self.global_best_position, self.global_best_score, self.convergence_history_per_iter
...
def run_pso(history_list):
    # Objective function must accept a continuous vector in [0,1]^dim
    def obj(x):
        return objective_function(x)

    pso = SimplePSO(obj_func=obj,
                    dim=HPO_DIM,
                    bounds=(np.zeros(HPO_DIM), np.ones(HPO_DIM)),
                    num_particles=POP_SIZE,
                    max_iters=int(MAX_FEVALS / POP_SIZE)) # e.g., 50 evals / 10 pop = 5 iters

    best_pos, best_score, conv_hist_per_iter = pso.optimize()

    # --- History Formatting ---
    # We must "stretch" the per-iteration history (conv_hist_per_iter) 
    # to a per-evaluation history (history_list) for a fair comparison.
    
    current_best = np.inf
    if not conv_hist_per_iter: # Failsafe
        conv_hist_per_iter = [best_score]

    # Stretch the per-iteration history
    for iteration_best_score in conv_hist_per_iter:
        if iteration_best_score < current_best:
            current_best = iteration_best_score
            
        # Add one history point for each particle (evaluation) in that iteration
        history_list.extend([current_best] * POP_SIZE)
